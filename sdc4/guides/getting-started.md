# Getting Started with SDC4

Welcome to SDC4! This guide will help you understand and start using the Semantic Data Charter.

## What is SDC4?

SDC4 (Semantic Data Charter version 4) is a framework for creating **semantically rich, interoperable data models**. It provides:
...
**Important**: `sdc4.xsd` is the normative source of truth.

```
sdc4.xsd → defines structure
Specification → describes structure
Examples → demonstrate usage
```

If documentation conflicts with `sdc4.xsd`, the schema is correct.

## Your First SDC4 Element
...
```bash
xmllint --schema sdc4/schemas/sdc4.xsd --noout patient-name.xml
```

If no errors, congratulations! You've created valid SDC4 data.
...
```bash
xmllint --schema sdc4/schemas/sdc4.xsd --noout patient-vital-signs.xml
```
...
# Load schema
schema_doc = etree.parse('sdc4/schemas/sdc4.xsd')
schema = etree.XMLSchema(schema_doc)

# Validate document
doc = etree.parse('your-file.xml')
schema.assertValid(doc)
print("✅ Valid SDC4 document")
```
...
- **[Specification](../specification/sdc4-specification.md)** - Complete reference
- **[Examples](../examples/)** - More working examples
- **[Architecture](../../docs/ARCHITECTURE.md)** - How SDC4 works
- **[FAQ](../../docs/FAQ.md)** - Common questions

### Build Your Own

1. **Identify your domain** - Healthcare? Finance? IoT?
2. **Choose your types** - Which SDC4 types fit your data?
3. **Create schema** - Use XSD restriction pattern
4. **Validate** - Test against `sdc4.xsd`
...
**You're ready to start building with SDC4!** 🚀
